...
double Agent::distance(const Agent &_agent) const
{
    return(sqrt(CGAL::squared_distance(this->_position,_agent._position)));
}
void Agent::update(const Point2D &_position,const Vector2D &_direction)
{
    this->_position=_position;
    this->_direction=_direction;
}
void Agent::direction(const Vector2D &_direction)
{
    this->_direction=_direction;
}
void Agent::position(const Point2D &_position)
{
    this->_position=_position;
}
